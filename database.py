"""
Async SQLite data-access layer for the Pick'em bot.
Every function opens its own short-lived connection (fine for bot-scale traffic).
"""
import datetime as dt
import aiosqlite

from config import DB_PATH, POINTS_WINNER, POINTS_SCORE, POINTS_MVP, SUBSCRIBE_BONUS
...
CREATE TABLE IF NOT EXISTS channels (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    chat_username TEXT NOT NULL,      -- e.g. @es1zeman  (must be public, or bot must be admin there)
...
# ------------------------------------------------------------- channels ----
async def add_channel(chat_username: str, display_name: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def set_active_tournament(tournament_id: int):
    """Only one tournament active at a time (mirrors the single Pick'em card in the UI)."""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def set_match_result(match_id: int, winner: str, score1: int, score2: int, mvp: str):
    """Finalize a match, then score every prediction placed on it."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        await db.execute(
        ...
        for p in preds:
            ...
            await db.execute(
                "UPDATE predictions SET points_earned=?, scored=1 WHERE id=?",
                (earned, p["id"]),
            )
            if earned:
                await db.execute(
...
async def list_daily_tasks(limit: int = 15) -> list[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cur = await db.execute(
            "SELECT * FROM daily_tasks ORDER BY id DESC LIMIT ?", (limit,)
        )
        return [dict(r) for r in await cur.fetchall()]
